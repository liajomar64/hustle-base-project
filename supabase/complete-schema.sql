-- Complete HustleBase Database Schema
-- This file creates everything from scratch - run this in Supabase SQL Editor
-- It's safe to run multiple times as it drops existing objects first
...
-- Drop all policies (IF EXISTS handles non-existent tables gracefully)
...
DROP POLICY IF EXISTS "Anyone can view job requests" ON job_requests;
DROP POLICY IF EXISTS "Clients can create job requests" ON job_requests;
DROP POLICY IF EXISTS "Clients can update own job requests" ON job_requests;
DROP POLICY IF EXISTS "Clients can delete own job requests" ON job_requests;

DROP POLICY IF EXISTS "Anyone can view job applications" ON job_applications;
DROP POLICY IF EXISTS "Providers can create applications" ON job_applications;
DROP POLICY IF EXISTS "Providers can update own applications" ON job_applications;
DROP POLICY IF EXISTS "Clients can update applications for their jobs" ON job_applications;
